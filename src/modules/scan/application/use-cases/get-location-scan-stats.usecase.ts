...
@Injectable()
export class GetLocationScanStatsUseCase {
  constructor(
  ...
  async execute(idQr: string, tracking: TrackingContext): Promise<unknown> {
    this.traceService.log(tracking, TraceLayer.USE_CASE, 'GetLocationScanStatsUseCase - input', {
      idQr,
    });
    return this.reader.getLocationStats(idQr, tracking);
  }
}
