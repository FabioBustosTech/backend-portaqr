...
@Injectable()
export class GetOriginScanStatsUseCase {
  constructor(
  ...
  async execute(idQr: string, tracking: TrackingContext): Promise<unknown> {
    this.traceService.log(tracking, TraceLayer.USE_CASE, 'GetOriginScanStatsUseCase - input', {
      idQr,
    });
    return this.reader.getOriginStats(idQr, tracking);
  }
}
